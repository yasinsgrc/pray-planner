  ...
  try {
    ...
    for (const scenario of SCENARIOS) {
      ...
      for (const tab of TABS) {
        ...
        // FadeIn sections use motion's whileInView (IntersectionObserver),
        // which only fires on a real scroll — Chromium's fullPage capture
        // (CDP captureBeyondViewport) never scrolls, so below-the-fold
        // FadeIn content stays stuck at its initial opacity:0 forever in
        // both the saved screenshot and any measurement taken after it.
        // A real user scrolling down triggers it normally; simulate that
        // once per tab (viewport:{once:true} means it then stays revealed
        // even after scrolling back to the top for the rest of the checks).
        ...
        // 5: touch target size — every visible, enabled clickable control
        // must be >=44x44 (its own box, or an invisible ::before expansion
        // per design-refresh-v3 Faz 2 F2).
        ...
        for (const issue of touchIssues) {
          violations.push(
            `[${scenario.name}/${tab.id}] touch target too small: <${issue.tag} class="${describeElement(issue.cls)}"` +
              ` label="${issue.label}"> ${issue.w.toFixed(0)}x${issue.h.toFixed(0)}px < ${MIN_TOUCH_TARGET}x${MIN_TOUCH_TARGET}px`
          );
        }

        // 6: text contrast — every visible text-bearing element must clear
        // WCAG AA (4.5:1 normal text, 3.0:1 for >=24px or >=18.66px bold).
        // Background is determined by pixel-sampling a real screenshot
        // (not by reasoning about color-mix/opacity in isolation): every
        // text-bearing element's own color is set to transparent, a
        // fullPage screenshot is taken, and the pixel(s) behind each
        // element's box are sampled from that screenshot — so translucent
        // layers, color-mix backgrounds, and glass blur are already fully
        // resolved by the browser's own compositor by the time we sample.
        await page.evaluate(() => window.scrollTo(0, 0));
        const textEls = await page.evaluate(() => {
          ...
          // getComputedStyle's raw string is not safe: Tailwind v4's
          // default palette (e.g. emerald-*) is OKLCH-defined, and Chromium
          // serializes getComputedStyle().color as "oklch(...)" for those,
          // which an rgb()-only regex silently misreads as black.
          ...
          document.querySelectorAll('body *').forEach((el, index) => {
            ...
            // position:fixed elements against document-flow content that
            // isn't actually behind them for a real scrolling user, so any
            // pixel sampled "through" its glass-panel blur here would be
            // sampling a background that doesn't correspond to reality.
        ...
        if (textEls.length > 0) {
          ...
          // Same fullPage-stitching artifact as the saved screenshot: the
          // fixed Navbar must be hidden here too, or it gets stamped over
          // document-flow content near the first-viewport-height mark and
          // corrupts background sampling for whatever real element sits
          // "under" that stamped copy (not under the real, once-only navbar
          // a scrolling user actually sees).
          await page.addStyleTag({ content: '[role="tablist"] { visibility: hidden !important; }' });
          // Elements with a `transition-colors` class (e.g. SegmentedControl)
          // don't apply color:transparent instantly — the change is the
          // start of a CSS transition, so a screenshot taken without
          // waiting for a repaint still shows the pre-hide color. Force
          // every transition to be instant for this capture only.
          ...
          for (const t of textEls) {
            ...
            // Kept well inside the box (not near corners/edges, which can
            // be antialiased or fall outside a rounded-corner background
            // like a selected SegmentedControl pill) so every sample lands
            // on the same surface as the glyphs themselves did.
            const offsets = [
              [0, 0],
            ...
            if (ratio < minRatio) {
              violations.push(
                `[${scenario.name}/${tab.id}] low contrast: <${t.tag} class="${describeElement(t.cls)}"> ` +
      ...
      await page.waitForTimeout(500);
      const zikirmatikPath = path.join(OUT_DIR, `${scenario.name}-zikirmatik.png`);
      await page.screenshot({ path: zikirmatikPath, fullPage: true });
      console.log(`  Saved ${zikirmatikPath}`);
      const zikirmatikOverflow = await page.evaluate(() => {
        const issues = [];
        const clientWidth = document.documentElement.clientWidth;
        document.querySelectorAll('body *').forEach((el) => {
          if (el.classList.contains('sr-only')) return;
      ...
      for (const issue of zikirmatikOverflow) {
        violations.push(
          `[${scenario.name}/zikirmatik] ${issue.kind}: <${issue.tag} class="${describeElement(issue.cls)}"> ` +
            `value=${issue.value.toFixed?.(1) ?? issue.value} clientWidth=${issue.clientWidth ?? 'n/a'}`
        );
      }
